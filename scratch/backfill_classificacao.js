...
async function run() {
    ...
    let sgc2Pool = null;
    try {
        console.log('Obtendo classificações para preencher a base histórica...');
        ...
        async function updateEmpresa(pool, empresa, codigos) {
            ...
            for (const chunk of chunks) {
                ...
                const query = `
                    SELECT p.codigo, cp.nome as classificacao
                    FROM produto p
                ...
                for (const row of msRes.recordset) {
                    if (row.classificacao) {
                        ...
                        `, [row.classificacao, empresa, row.codigo]);
                    }
                }
        ...
        console.log('Preenchimento retroativo (backfill) concluído com sucesso!');
