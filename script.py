...
def main():
    ...
    try:
        ...
        for table in TABLES:
            ...
            # Converter dados brutos para tuplas e limpar lixo ("porteiro")
            access_data = []
            garbage_values = {"-", "/", "'", "0", ""}
                ...
                if table_name == "Cirurgias":
                    ...
                    for idx, col_name in enumerate(columns):
                        if col_name in clean_cols:
                            val = row_list[idx]
                            if isinstance(val, str) and val.strip() in garbage_values:
                                row_list[idx] = None
                            elif col_name == "pedido" and val == 0:
                                row_list[idx] = None
                access_data.append(tuple(row_list))
            ...
            if access_data:
                ...
                if update_cols:
                    ...
                    for c in update_cols:
                        if table_name == "Cirurgias" and c in ["pedido", "nota_fiscal"]:
                            update_str_parts.append(f"{c} = CASE WHEN EXCLUDED.{c} IS NULL OR TRIM(CAST(EXCLUDED.{c} AS TEXT)) IN ('', '0', '0.0') THEN {table_name}.{c} ELSE EXCLUDED.{c} END")
                        else:
