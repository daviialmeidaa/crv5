...
(case isnull(nota_fiscal_venda.id_situacao_nfe, nota_fiscal_venda.situacao) 
 (3) then nota_fiscal_venda.situacao 
 (5) then nota_fiscal_venda.situacao 
 (6) then nota_fiscal_venda.situacao 
isnull(nota_fiscal_venda.id_situacao_nfe, nota_fiscal_venda.situacao) 
end) 
when (1) then 'Emitida'
...
,case tipo_nota_fiscal.ind_venda 
 ...
 2 then 'Devolução'
 3 then 'Bonificação'
'Outras' end as tipo_nota_operacao
...
,case produto.id_classificacao
 ...
 1 then 'Genérico'
 2 then 'Similar'
 3 then 'Outros' end as categoria_medicamento
,case.produto.id_lista_pis_cofins 
 2 then 'Negativa'
 1 then 'Positiva'
 3 then 'Neutra' end as Lista_Pis_Cofins
    ,produto.codigo
 ...
 * (select case nota_fiscal_venda.id_desconto_acrescimo when 1 then -1 else 1 end)) as rateio_desc_acres 
 -------------------------------------------------------------------------------- -- precisão alterada para 4 casas em 24/04/2017
  ...
  (Convert(decimal(12,4),nota_fiscal_venda_item.valor_unitario))) + 
(convert(decimal(12,4),
ull(nota_fiscal_venda_item_lote.quantidade,nota_fiscal_venda_item.quantidade) * 
  (Convert(decimal(12,4),nota_fiscal_venda_item.valor_unitario)) /   
isnull(nullif((select sum(isnull(nlote.quantidade,nitem.quantidade) * 
  (Convert(decimal(12,4),nitem.valor_unitario))) 
 from nota_fiscal_venda_item nitem 
...
,CASE nota_fiscal_venda.id_desconto_acrescimo 
 2 THEN (nota_fiscal_venda_item.valor_total + 
ull(nota_fiscal_venda_item.valor_desc_acresc_rateio_sintegra, 0)    
...
(nota_fiscal_venda_item.valor_total - 
ull(nota_fiscal_venda_item.valor_desc_acresc_rateio_sintegra, 0) 
isNull(nota_fiscal_venda_item.valor_substituicao_tributaria, 0) 
isNull(nota_fiscal_venda_item.valor_icms_substituicao_reembolso_rateio, 0) 
isNull(nota_fiscal_venda_item.valor_despesa_acessoria_rateio,0) 
...
nota_fiscal_venda join nota_fiscal_venda_item on nota_fiscal_venda_item.nf_numero = nota_fiscal_venda.codigo
 produto on produto.codigo = nota_fiscal_venda_item.prod_codigo
 fabricante on fabricante.codigo = produto.fabr_codigo
 cliente_fornecedor on cliente_fornecedor.codigo = nota_fiscal_venda.clifor_codigo
 tipo_nota_fiscal on tipo_nota_fiscal.codigo = nota_fiscal_venda.tiponf_codigo
join nota_fiscal_venda_item_lote on (nota_fiscal_venda_item.nf_numero = nota_fiscal_venda_item_lote.nfit_nf_numero
        d nota_fiscal_venda_item.codigo = nota_fiscal_venda_item_lote.nfit_codigo)
join vendedor on vendedor.codigo = nota_fiscal_venda.vend_codigo
join condicao_pagamento on condicao_pagamento.codigo = nota_fiscal_venda.condpg_codigo
join cobranca on cobranca.codigo = nota_fiscal_venda.cob_codigo
join cidade on cidade.codigo = cliente_fornecedor.cid_codigo
join regiao on regiao.codigo = cliente_fornecedor.regi_codigo
join cliente_fornecedor as Transp on Transp.codigo = nota_fiscal_venda.tran_codigo
join classificacao_cliente as clascli on clascli.codigo = cliente_fornecedor.clascli_codigo_1
join classificacao_produto as claspro on claspro.codigo = produto.claspro_codigo_1
join produto_unidade_conversao puc on puc.prod_codigo = nota_fiscal_venda_item.prod_codigo and puc.unidade_convertida = nota_fiscal_venda_item.unidade 
join vendedor atendente on atendente.codigo = nota_fiscal_venda.vend_codigo_2
join nota_fiscal_venda nfv1 on nfv1.codigo = nota_fiscal_venda.nf_codigo_complementada
...
async function run() {
    try {
        ...
        console.log('Altering view on SGC...');
        await pool.request().query('USE SGC;');
        await pool.request().query(sgcView);
        console.log('Successfully altered view on SGC!');

        console.log('Altering view on SGC2...');
        await pool.request().query('USE SGC2;');
        await pool.request().query(sgcView);
        console.log('Successfully altered view on SGC2!');
